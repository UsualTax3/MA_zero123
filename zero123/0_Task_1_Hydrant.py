...
def load_and_initialize_models():
    global models
    ckpt_path = "/export/scratch/ru89tew/logs/2024-05-04T23-17-33_sd-objaverse-finetune-c_concat-256/checkpoints/epoch=000004.ckpt"
...
def preprocess_image(models, input_im, preprocess):
    """
    Preprocess the input image, handling both RGB and RGBA images correctly.

    :param models: A dictionary containing the models used in preprocessing.
    :param input_im: The input image to preprocess.
    :param preprocess: A boolean flag indicating whether additional preprocessing is needed.
    :return: The preprocessed image as a (H, W, 3) array in [0, 1].
    """
    ...
    if preprocess:
        ...
        # Handling RGB and RGBA images
        if input_im.shape[-1] == 4:  # RGBA
            alpha = input_im[:, :, 3:4]  # Extract the alpha channel
            white_background = np.ones_like(input_im[:, :, :3])  # Create a white background for RGB
            input_im = alpha * input_im[:, :, :3] + (1.0 - alpha) * white_background  # Blend the image with the white background
        elif input_im.shape[-1] == 3:  # RGB
            # No alpha channel to process, but you might add RGB specific preprocessing here if necessary
            pass
        else:
            raise ValueError(f"Unsupported image shape: {input_im.shape}. Expected RGB or RGBA.")

    print(f'Preprocessing took {time.time() - start_time:.3f}s.')
...
if __name__ == "__main__":
    ...
    while len(processed_sequences) < 5:
        ...
        for frame_detail in subsequent_frames:
            ...
            preprocessed_corresponding_img_array = preprocess_image(models, corresponding_img, preprocess=False)
