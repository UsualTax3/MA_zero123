...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compute Perceptual Similarity, PSNR, and SSIM for image pairs across multiple folders.")
    parser.add_argument("--take_every_other", action="store_true", default=False, help="Whether to take every other image or process all images.")
    ...
    for folder_name in folder_names:
        folder_path = os.path.join('/export/compvis-nfs/user/rbarlog/zero123/zero123/', folder_name)
        ...
        for i in range(1, 12):
            pred_img_path = os.path.join('/export/compvis-nfs/user/rbarlog/zero123/zero123/', 'Predict/', folder_name, f'{i}_pred.png')
            tgt_img_path = os.path.join('/export/compvis-nfs/user/rbarlog/zero123/zero123/', 'GT/', folder_name, f'{i}_target.png')
            
            print(pred_img_path)
            print(tgt_img_path)
            
            if os.path.exists(pred_img_path) and os.path.exists(tgt_img_path):
                pred_imgs_list.append([pred_img_path])
                tgt_imgs_list.append(tgt_img_path)
            else:
                print(f"Warning: Pair {i}_pred.png and {i}_target.png not found in {folder_name}, skipping.")

        if pred_imgs_list and tgt_imgs_list:
            folder_results = compute_perceptual_similarity_from_list(
                pred_imgs_list, tgt_imgs_list, opts.take_every_other, simple_format=True
            )
            results["Perceptual similarity"].append(folder_results["Perceptual similarity"][0])
            results["PSNR"].append(folder_results["PSNR"][0])
            results["SSIM"].append(folder_results["SSIM"][0])
            print(f"Results for {folder_name}:")
            print("Perceptual Similarity:", folder_results["Perceptual similarity"])
            print("PSNR:", folder_results["PSNR"])
            print("SSIM:", folder_results["SSIM"])
        else:
            print(f"No valid image pairs found in {folder_name}.")

    # Compute mean and standard deviation across all folders
    mean_percsim = np.mean(results["Perceptual similarity"])
    mean_psnr = np.mean(results["PSNR"])
    mean_ssim = np.mean(results["SSIM"])

    std_percsim = np.std(results["Perceptual similarity"])
    std_psnr = np.std(results["PSNR"])
    std_ssim = np.std(results["SSIM"])

    print("\nFinal aggregated results across all folders:")
    print(f"Mean Perceptual Similarity: {mean_percsim} ± {std_percsim}")
    print(f"Mean PSNR: {mean_psnr} ± {std_psnr}")
    print(f"Mean SSIM: {mean_ssim} ± {std_ssim}")