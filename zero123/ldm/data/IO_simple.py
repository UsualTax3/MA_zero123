...
class ObjaverseDataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, root_dir, batch_size, total_view, train=None, validation=None, test=None, num_workers=4, **kwargs):
        super().__init__()
        ...
        if 'image_transforms' in dataset_config:
            image_transforms = [transforms.Resize((dataset_config['image_transforms']['size'], dataset_config['image_transforms']['size']))]
        else:
            image_transforms = []
    ...
    def make_dataset(self, validation=False):
        dataset = ObjaverseData(root_dir=self.root_dir, total_view=self.total_view, validation=validation, image_transforms=self.image_transforms)
        return dataset

    def train_dataloader(self):
        dataset = self.make_dataset(validation=False)
        sampler = DistributedSampler(dataset)
        print("Creating train DataLoader")
        return DataLoader(dataset, batch_size=self.batch_size, num_workers=self.num_workers, sampler=sampler)

    def val_dataloader(self):
        dataset = self.make_dataset(validation=True)
        sampler = DistributedSampler(dataset)
        return DataLoader(dataset, batch_size=self.batch_size, num_workers=self.num_workers, sampler=sampler)
...
class ObjaverseData(Dataset):
    def __init__(self, root_dir, image_transforms=[], total_view=12, validation=False):
        self.root_dir = root_dir
        self.total_view = total_view
        self.validation = validation

        # Load valid paths from JSON
        with open('/export/compvis-nfs/user/rbarlog/zero123/zero123/valid_paths.json') as f:
            self.paths = json.load(f)
        
        total_objects = len(self.paths)
        if validation:
            self.paths = self.paths[math.floor(total_objects / 100. * 99.):]  # Use last 1% as validation
        else:
            self.paths = self.paths[:math.floor(total_objects / 100. * 99.)]  # Use first 99% as training

        print('============= length of dataset %d =============' % len(self.paths))
        self.tform = transforms.Compose(image_transforms)
        
        # Initialize WebDataset
        self.dataset = iter(wds.WebDataset(root_dir).decode("pil").to_tuple("png", "npy"))
    ...
    def extract_file_from_tar(self, tar, filename):
        try:
            file_obj = tar.extractfile(filename)
            if file_obj is not None:
                return file_obj.read()
            else:
                raise FileNotFoundError(f"File {filename} not found in tar archive.")
        except Exception as e:
            print(f"Error extracting {filename} from tar: {e}")
            raise
    ...
    def __getitem__(self, index):
        ...
        folder_name = self.paths[index]

        print(f"Accessing folder: {folder_name}")

        try:
            with tarfile.open(self.root_dir, 'r:gz') as tar:
                target_im_data = self.extract_file_from_tar(tar, os.path.join(folder_name, '%03d.png' % index_target))
                cond_im_data = self.extract_file_from_tar(tar, os.path.join(folder_name, '%03d.png' % index_cond))
                target_RT_data = self.extract_file_from_tar(tar, os.path.join(folder_name, '%03d.npy' % index_target))
                cond_RT_data = self.extract_file_from_tar(tar, os.path.join(folder_name, '%03d.npy' % index_cond))

                target_im = self.process_im(Image.open(io.BytesIO(target_im_data)))
                cond_im = self.process_im(Image.open(io.BytesIO(cond_im_data)))
                target_RT = np.load(io.BytesIO(target_RT_data))
                cond_RT = np.load(io.BytesIO(cond_RT_data))
        except Exception as e:
            print(f"Error loading images or RT files: {e}")
            # Fallback to a known valid folder
            with tarfile.open(self.root_dir, 'r:gz') as tar:
                target_im_data = self.extract_file_from_tar(tar, os.path.join('692db5f2d3a04bb286cb977a7dba903e_1', '%03d.png' % index_target))
                cond_im_data = self.extract_file_from_tar(tar, os.path.join('692db5f2d3a04bb286cb977a7dba903e_1', '%03d.png' % index_cond))
                target_RT_data = self.extract_file_from_tar(tar, os.path.join('692db5f2d3a04bb286cb977a7dba903e_1', '%03d.npy' % index_target))
                cond_RT_data = self.extract_file_from_tar(tar, os.path.join('692db5f2d3a04bb286cb977a7dba903e_1', '%03d.npy' % index_cond))
...
class Co3dData(Dataset):
    ...
    def _load_samples(self):
        ...
        for i, (sequence_name, frame_number, image_path) in enumerate(all_samples):
            ...
            if sequence_frame_count[sequence_name] < 2:
    ...
    def __getitem__(self, index):
        ...
        try:
            while not image_loaded:
                ...
                #print(f'Target_path {target_im}')
                #print(f'Cond_path {cond_im}')
                #print(f"Target_RT {target['image']['path']}")
                #print(f"Target_RT {cond['image']['path']}")
                
                #print(target)
                #print(cond)
                
                #base_directory = 'co3d/CO3DV2_DATASET_ROOT'  # Adjust this to your actual base directory
                #full_image_path_target = f"{base_directory}/{target['image']['path']}"
                #full_image_path_cond = f"{base_directory}/{cond['image']['path']}"
                
                #print()
                #print(target_path)
                #print(cond_path)
                #print(full_image_path_target)
                #print(full_image_path_cond)
                #print()
                
                # Set up the figure and axes for plotting
                #fig, axs = plt.subplots(1, 4, figsize=(20, 5)) # Adjust figsize as needed

                #Now, open each image and display it on the respective subplot
                #axs[0].imshow(Image.open(target_path))
                #axs[0].axis('off')

                #axs[1].imshow(Image.open(cond_path))
                #axs[1].axis('off')

                #axs[2].imshow(Image.open(full_image_path_target))
                #axs[2].axis('off')

                #axs[3].imshow(Image.open(full_image_path_cond))
                #axs[3].axis('off')

                #plt.show()
                
                if(filename == '216_22871_50171'):
                    print('216_22871_50171')
                    print(target_path)
                    print(cond_path)
                
                #print(target_path)
                #print(cond_path)
                #print(target['viewpoint'])
                #print(cond['viewpoint'])
                image_loaded = True
            ...
            #print(target_viewpoint)
            #print(cond_viewpoint)
            #print(target_RT)
            #print(cond_RT)
        ...
        #axs[1].imshow(Image.open(cond_im))
        #axs[1].axis('off')
