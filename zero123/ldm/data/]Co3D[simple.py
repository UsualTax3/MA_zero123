...
class Co3dDataModuleFromConfig(pl.LightningDataModule):
    ...
    def val_dataloader(self):
        dataset = Co3dDataVAL(root_dir=self.root_dir, total_view=self.total_view, validation=True, \
                                image_transforms=self.image_transforms)
        sampler = DistributedSampler(dataset)
        return wds.WebLoader(dataset, batch_size=self.batch_size, num_workers=self.num_workers, shuffle=False)
    ...
    def __getitem__(self, index):
        ...
        try:
            while not image_loaded:
                ...
                if not os.path.exists(target_path) or not os.path.exists(cond_path) or not os.path.exists(target_mask) or not os.path.exists(cond_mask):
                    continue
                ...
                mask_target = cv2.imread(target_mask, cv2.IMREAD_GRAYSCALE)
                
                image_cond = cv2.imread(cond_path)
                mask_cond = cv2.imread(cond_mask, cv2.IMREAD_GRAYSCALE)
                ...
                mask_bool_cond = mask_cond > 128
                white_background_cond = np.ones_like(image_cond) * 255
                result_image_cond = np.where(mask_bool_cond[:, :, np.newaxis], image_cond, white_background_cond)
                cond_im = cv2.cvtColor(result_image_cond, cv2.COLOR_BGR2RGB)
                
                # Resize the image to 256x256 pixels
                #cond_im_resized = cv2.resize(cond_im, (256, 256))
                #cond_im_resized = cv2.resize(cond_im, (256, 256))

                #print(cond_im_resized.shape)  # This should print: (256, 256, 3)

                # Display the resized image
                #plt.imshow(cond_im_resized)
                #plt.axis('off')  # Optional: to hide the axis for a cleaner presentation
                #plt.show()
                ...
                # Replace the background with white, ensuring to apply only to the RGB channels
                #target_im[:, :, :3] = np.where(target_im[:, :, 3:] == 0, color_2[:3], target_im[:, :, :3])
                #cond_im[:, :, :3] = np.where(cond_im[:, :, 3:] == 0, color_2[:3], cond_im[:, :, :3])

                # Convert images from RGBA to RGB (if you added an alpha channel to images without it)
                #target_im = Image.fromarray(target_im[:, :, :3].astype('uint8'), 'RGB')
                #cond_im = Image.fromarray(cond_im[:, :, :3].astype('uint8'), 'RGB')
...
class Co3dDataVAL(Dataset):
    def __init__(self,
        ...
        self.postprocess = postprocess
        self.total_view = total_view
        self.annotations = self.open_gzipped_json('co3d/CO3DV2_DATASET_ROOT/chair/frame_annotations.jgz')
        self.samples = self._load_samples()
        #self.subset_file = 'chair/set_lists.json'
        self.tform = image_transforms

    # ['429_60517_117787', 100, 'co3d\\CO3DV2_DATASET_ROOT\\chair/429_60517_117787/images/frame000101.jpg'], 
    # ['429_60517_117787', 101, 'co3d\\CO3DV2_DATASET_ROOT\\chair/429_60517_117787/images/frame000102.jpg']]
    #def _load_samples(self):
        #subset_path = os.path.join(self.root_dir, 'chair/set_lists.json')
        #with open(subset_path, 'r') as f:
            #subsets = json.load(f)
    ...
    def __getitem__(self, index):
        ...
        try:
            while not image_loaded:
                ...
                index_target += 1
                index_cond += 1 
                target_path = os.path.join("co3d", "CO3DV2_DATASET_ROOT", "chair", filename, "images", f"frame000{index_target:03d}.jpg")
                cond_path = os.path.join("co3d", "CO3DV2_DATASET_ROOT", "chair", filename, "images", f"frame000{index_cond:03d}.jpg") 
                
                target_mask = os.path.join("co3d", "CO3DV2_DATASET_ROOT", "chair", filename, "masks", f"frame000{index_target:03d}.png")
                cond_mask = os.path.join("co3d", "CO3DV2_DATASET_ROOT", "chair", filename, "masks", f"frame000{index_cond:03d}.png")
                ...
                annotations = self.open_gzipped_json(os.path.join("co3d", "CO3DV2_DATASET_ROOT", "chair", 'frame_annotations.jgz'))
                #print(annotations['image']['path'])
                #print(target_path)    
