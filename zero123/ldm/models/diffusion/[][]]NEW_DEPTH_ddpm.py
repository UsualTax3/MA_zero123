...
class ColorHistogramLoss(nn.Module):
    def __init__(self, num_bins=256, min=0.0, max=1.0):
    ...
    def forward(self, img1, img2):
        ...
        histogram_loss = F.mse_loss(hist1, hist2)
    ...
    def compute_histogram(self, img):
        ...
        for i in range(batch_size):
            ...
            for c in range(channels):
                channel_data = img[i, c].view(-1)
                hist_channel = torch.histc(
                    channel_data, bins=self.num_bins, min=self.min, max=self.max
                )
                hist_channel = hist_channel.float() 
                hist_channel = hist_channel / (hist_channel.sum() + 1e-8)
...
class DepthConsistencyLoss(nn.Module):
    ...
    def normalize_depth(self, depth):
        min_d = depth.amin(dim=[1, 2], keepdim=True)
    ...
    def forward(self, img1, img2):
        ...
        depth1 = self.normalize_depth(depth1)
        depth2 = self.normalize_depth(depth2)

        loss = F.mse_loss(depth1, depth2)
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.color_histogram_loss = ColorHistogramLoss()
        self.depth_estimation_loss = DepthConsistencyLoss()
        ...
        self.perceptual_weight = 0.005
        self.color_weight = 0.005
        self.depth_weight = 0.005
...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        # Compute the color histogram loss
        color_loss = self.color_histogram_loss(x_reconstructed, x_start)
        weighted_color_loss = self.color_weight * color_loss
        loss += weighted_color_loss
        loss_dict.update({f'{prefix}/color_loss': color_loss})
        loss_dict.update({f'{prefix}/weighted_color_loss': weighted_color_loss})
        
        depth_loss = self.depth_estimation_loss(x_reconstructed, x_start)
        weighted_depth_loss = self.depth_weight * depth_loss
        loss += weighted_depth_loss
        loss_dict.update({f'{prefix}/depth_loss': depth_loss})
        loss_dict.update({f'{prefix}/weighted_depth_loss': weighted_depth_loss})
