...
class PerceptualLoss(nn.Module):
    def __init__(self, layers=['conv1_1', 'conv2_1', 'conv3_1', 'conv4_1', 'conv5_1']):
        ...
        for name in layers:
            self.slices.append(vgg_slices[name])

        for param in self.parameters():
            param.requires_grad = False

    def forward(self, x, y):
        print(f"PerceptualLoss forward: initial x shape: {x.shape}, y shape: {y.shape}")
        loss = 0.0
        for i, layer in enumerate(self.slices):
            x = layer(x)
            y = layer(y)
            print(f"Layer {i}: x shape after layer: {x.shape}, y shape after layer: {y.shape}")
...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        # Ensure both tensors have 3 channels and resize x_start if necessary
        if x_start.shape[1] == 4:  # Assuming that 4 channels indicate the need for decoding
            x_start = self.decode_first_stage(x_start)
            #print(f"Decoded x_start shape: {x_start.shape}")

        # Debug prints for shape verification
        #print(f"x_reconstructed shape after decoding: {x_reconstructed.shape}")
        #print(f"x_start shape after decoding: {x_start.shape}")

        # Apply PerceptualLoss
        perceptual_loss = self.perceptual_loss(x_reconstructed, x_start).mean()
