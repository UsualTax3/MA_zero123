    ...
    def compute_histogram(self, img):
        ...
        for i in range(batch_size):
            ...
            for c in range(channels):
                hist_channel = torch.histc(img[i, c], bins=self.num_bins, min=0, max=1)
...
class DepthConsistencyLoss(nn.Module):
    ...
    def forward(self, img1, img2):
        # Apply transformations (typically on CPU)
        img1 = self.transform(img1)
        img2 = self.transform(img2)

        print(f"After transformation, img1 is on device: {img1.device}")
        print(f"After transformation, img2 is on device: {img2.device}")

        # Perform inference with no gradient tracking (on GPU if model is on GPU)
        with torch.no_grad():
            depth1, _, _ = self.depth_estimator.inference({'input': img1})
            depth2, _, _ = self.depth_estimator.inference({'input': img2})

        print(f"After inference, depth1 is on device: {depth1.device}")
        print(f"After inference, depth2 is on device: {depth2.device}")

        # Calculate MSE loss (on GPU if depth1 and depth2 are on GPU)
        loss = F.mse_loss(depth1, depth2)
        print(f"Loss is calculated on device: {loss.device}")
