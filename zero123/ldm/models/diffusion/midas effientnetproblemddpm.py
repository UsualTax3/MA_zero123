...
class DepthConsistencyLoss(nn.Module):
    def __init__(self, model_type="MiDaS_small", device='cuda'):
        super(DepthConsistencyLoss, self).__init__()
        self.device = device

        # Specify the local path to the cached MiDaS model
        local_path = "/export/compvis-nfs/user/rbarlog/.cache/torch/hub/intel-isl_MiDaS_master"

        # Load MiDaS model from the local cache
        self.depth_estimator = torch.hub.load(
            repo_or_dir=local_path,
            model=model_type,
            source='local'  # Load from the local path
        )
        ...
        self.transform = T.Compose([
            T.Resize((256, 256)),  # Resize to a smaller size appropriate for MiDaS_small
...
class CombinedLoss(nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        total_loss = perceptual_loss + color_loss + depth_loss
        return total_loss, {
