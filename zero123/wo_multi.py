...
_GPU_INDEXES = [0]  # Default to the first GPU; modify as needed for multi-GPU


def load_model_from_config(config, ckpt, device, device_idxs, verbose=False):
    ...
    model.to(device)
    if len(device_idxs) > 1:  # Multi-GPU setup
        model = torch.nn.DataParallel(model, device_ids=device_idxs)
    model.eval()
    return model
...
def predict(device_idxs: list = _GPU_INDEXES,
    ...
    models = dict()
    models['turncam'] = load_model_from_config(config, ckpt, device=device, device_idxs=device_idxs, verbose=True)
